...
{
    /**
     * @var Finder Singleton master instance
     */
    protected static $instance;

    /**
     * @var array Holds all of the search paths
     */
    protected $paths = [];

    /**
     * @var array Search paths that only last for one lookup
     */
    protected $flash_paths = [];

    /**
     * @var int the amount of time to cache in seconds
     */
    protected $cache_lifetime;

    /**
     * @var string path to the cache file location
     */
    protected $cache_dir;

    /**
     * @var array Cached lookup paths
     */
    protected $cached_paths = [];

    /**
     * @var bool Whether the path cache is valid or not
     */
     ...
     */
    public static function search(string $dir, string $file, string $ext = '.php', bool $multiple = false, bool $cache = true) : mixed
    {
     ...
     * @return Finder
     */
    public static function instance() : Finder
    {
        if ( ! static::$instance)
        {
            static::$instance = static::forge([APPPATH]);
        }
     ...
     * @return Finder
     */
    public static function forge(array $paths = []) : Finder
    {
     ...
     * @return $this
     */
    public function add_path($paths, ?int $pos = null)
    {
        // Add package path config
     ...
     * @return $this
     */
    public function flash(array $paths)
    {
        if ( ! is_array($paths))
        {
            $paths = [$paths];
     ...
     * @return string
     */
    public function prep_path(string $path) : string
    {
     ...
     */
    public function list_files(?string $directory = null, string $filter = '*.php') : array
    {
        $paths = $this->paths;
     ...
     * @return void
     */
    public function write_cache(string $cache_id) : void
    {
     ...
     */
    protected function from_cache(string $cache_id)
    {
     ...
     * @return string|bool Path or false if not found
     */
    protected function add_to_cache(string $cache_id, $path)
    {
        $cache_id                      = md5($cache_id);
     ...
     * @return bool|null
     */
    protected function cache(string $name, ?array $data = null, ?int $lifetime = null) : bool
    {
