 ...
 * @group Uri
 */
class Test_Uri extends \PHPUnit\Framework\TestCase
{
    ...
    public function test_create() : void
    {
        ...
        $output   = Uri::create('controller/:some', ['some' => 'thing', 'and' => 'more'], ['what' => ':and']);
        ...
        $output   = Uri::create('http://example.com/controller/:some', ['some' => 'thing', 'and' => 'more'], ['what' => ':and']);
        $expected = 'http://example.com/controller/thing.html?what=more';
        $this->assertEquals($expected, $output);

        $output   = Uri::create('http://example.com/controller/:some', ['some' => 'thing', 'and' => 'more'], ['what' => ':and'], true);
        $expected = 'https://example.com/controller/thing.html?what=more';
        $this->assertEquals($expected, $output);

        $output   = Uri::create('https://example.com/controller/:some', ['some' => 'thing', 'and' => 'more'], ['what' => ':and'], false);
        $expected = 'http://example.com/controller/thing.html?what=more';
        $this->assertEquals($expected, $output);
    }
     ...
     * @test
     */
    public function test_build_query_string() : void
    {
        $output   = Uri::build_query_string(['varA' => 'varA'], 'varB', ['varC' => 'varC']);
     ...
     * @test
     */
    public function test_update_query_string() : void
    {
        Config::set('base_url', 'http://example.com/test');
        Config::set('index_file', null);
        Config::set('url_suffix', '');
        Input::instance()->_set('get', ['one' => 1, 'two' => 2]);

        $output   = Uri::update_query_string(['three' => 3]);
        $expected = 'http://example.com/test?one=1&two=2&three=3';
        $this->assertEquals($expected, $output);

        $output   = Uri::update_query_string(['two' => 3]);
        $expected = 'http://example.com/test?one=1&two=3';
        $this->assertEquals($expected, $output);

        $output   = Uri::update_query_string(['four' => 4], 'http://localhost/controller');
